...
//			ImageDescriptor desc = Activator.getImageDescriptor("icons/checkedBox.png");
		...
		public Image getImage(Object obj) {
			...
			{
				if(cachedProgress.containsKey(((Task)obj)) && cachedProgress.get(((Task)obj)).isCompleted())
					return checkedImage;
				else
		...
		public Color getForeground(Object element) {
			...
			{
				Task t = (Task) element;
				if(cachedProgress.containsKey(t) && cachedProgress.get(t).isCompleted())
					return Display.getCurrent().getSystemColor(SWT.COLOR_GRAY);
				else if(((Quest) t.getQuest().getRef()).getAssignment().getDueOn() == null)
					return null;
				else if(Util.daysTillDate(t.getQuest().getAssignment().getDueOn()) < 3)
					return Display.getCurrent().getSystemColor(SWT.COLOR_RED);
				else if(Util.daysTillDate(t.getQuest().getAssignment().getDueOn()) < 7)
							...
							if((e.y > item.getImageBounds(0).y) && (e.y < (item.getImageBounds(0).y + item.getImage().getBounds().height))) {
								if(item.getData() instanceof Task)
								{
									...
									if(cachedProgress.containsKey(t))
										cachedProgress.get(t).setCompleted(!cachedProgress.get(t).isCompleted());
									else
									{
										cachedProgress.put(t, new QuestProgress());
										cachedProgress.get(t).setCompleted(true);
									}
